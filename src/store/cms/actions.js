  ...
  async fetchHomePage({commit}){
    ...
    if(response?.data){
      ...
      const home = {
        ...
        mor_services:res.mor_services?.map(service=>prismicH.asText(service.service_title)),
        ...
        partners:res.partners?.map(partner=>prismicH.asImageSrc(partner.partner_logo)),
